export const isDateValid = (startDate, endDate, currentlyWorking) => {
...
export const isDateValidAndRequired = (
  startDate,
  endDate,
  currentlyWorking,
) => {
...
export const isRequired = (value, fieldName) => {
  if (!value) {
    // Check if the field name has any uppercase letters indicating camelCase
    const hasUpperCase = /[A-Z]/.test(fieldName);
    const formattedFieldName = hasUpperCase
      ? fieldName
    ...
    return { valid: false, message: `${formattedFieldName} is required.` };
  }
  return { valid: true, message: "" };
};

export const isName = (value, fieldName) => {
  if (/\d/.test(value)) {
...
export const isPhoneNumberValid = (phoneNumber) => {
  ...
  const regex =
    /^(\+|00)?\s?(\(?\d{1,4}\)?)?\s?\d{1,4}\s?(\d{1,4}\s?){1,4}\d{1,4}$/;

  return regex.test(phoneNumber)
    ? { valid: true, message: "" }
    : { valid: false, message: "Phone number is invalid" };
};
...
export const isDobValid = (dob) => {
  ...
  return isEighteenOrOlder
    ? { valid: true, message: "" }
    : { valid: false, message: "Person must be at least 18 years old." };
};

export const isNumberAndRequired = (value, fieldName) => {
  if (!value) {
...
export const isSSN = (value, fieldName) => {
  //should always be 13 digits
  if (value.length !== 13) {
    return { valid: false, message: `${fieldName} should be 13 digits.` };
  }
  return { valid: true, message: "" };
}