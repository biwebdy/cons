...
import SubProposalCard from "../card/SubProposalCard";
import { getSubClientsProposals } from "@/data/proposals";

export default function SubClientProposals({ id, proposalsStatus }) {
  ...
  useEffect(() => {
    const getProposals = async () => {
      const resp = await getSubClientsProposals(id, proposalsStatus);
          ...
          <table className="min-w-full bg-white rounded-lg overflow-hidden shadow">
            ...
            <tbody>
              {proposalsData?.map((proposal, i) => (
                <SubProposalCard data={proposal} key={i} />
              ))}
            </tbody>
          </table>
          <div className="mt-8">
